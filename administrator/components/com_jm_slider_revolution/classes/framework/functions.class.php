...
class UniteFunctionsRev{
	...
	public static function importSliderHandle($viewBack = null, $updateAnim = true, $updateStatic = true){
		...
		if(empty($viewBack)){
			$viewBack = self::getViewUrl($sliderID,'edit','slider');
			if(empty($sliderID))
				$viewBack = self::getViewUrl();
		}
		...
		if($response["success"] == false){
			...
			echo self::getHtmlLink($viewBack,"Go Back");
		}
		else{	//handle success, js redirect.
			dmp("Slider Import Success, redirecting...");
			echo "<script> var url = '$viewBack'; url = url.replace(new RegExp('&amp;', 'g'), '&'); location.href=
			url;</script>";
		}
		exit();
	}
	...
	public static function getViewUrl($id = null,$layout = null,$view = 'sliders',$component = 'com_jm_slider_revolution'){
		$url = "index.php?option={$component}&view={$view}";
		if($layout){
			$url .="&layout={$layout}";
		}
		if($id){
	...
	public static function getImagePathFromURL($img){
		$result = str_replace(JURI::root(),'',$img);
		return $result;
	}
	
	public static function getImageUrlFromPath($img){
		$result = str_replace(JPATH_SITE.'/',JURI::root(),$img);
		return $result;
	}
	...
	public static function getUrlUploads(){
		...
		return(JURI::root().'images/revslider/');
	...
	public static function import_media($file_url, $folder_name) {
		...
		$artDir = self::getPathUploads();
		//if the directory doesn't exist, create it	
		if(!file_exists($artDir)) mkdir($artDir);
		if(!file_exists($artDir.$folder_name)) mkdir($artDir.$folder_name);
		...
		if(@fclose(@fopen($file_url, "r"))){ //make sure the file actually exists
			...
			$saveDir = $artDir.$folder_name.$filename;
			
			copy($file_url, $saveDir);
			
			$file_info = getimagesize($saveDir);
			return array("id" => 0, "path" => $saveDir);
		}else{
			return false;
		}
	}
