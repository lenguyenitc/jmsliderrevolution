...
class RevSlide extends UniteElementsBaseRev{
	...
	public function initByData($record){
		...
		$this->slideOrder = @$record["ordering"];
		...
		$this->imageFilepath = UniteFunctionsRev::getImagePathFromURL($this->imageUrl);
		$realPath = $this->imageFilepath;
	...
	public function getLayersForExport($useDummy = false){
		...
		foreach($this->arrLayers as $key=>$layer){
			...
			if(!empty($imageUrl))
				$layer["image_url"] = UniteFunctionsRev::getImagePathFromURL($layer["image_url"]);
	...
	public function getParamsForExport(){
		...
		if(!empty($urlImage))
			$arrParams["image"] = UniteFunctionsRev::getImagePathFromURL($urlImage);
