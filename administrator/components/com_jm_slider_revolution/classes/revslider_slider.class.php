...
class RevSlider extends UniteElementsBaseRev{
	...
	public static $UniteFunctionsRev;
	...
	private function createUpdateSliderFromOptions($options,$sliderID = null, $settingsMain, $settingsParams){
		...
		if(!empty($sliderID)) $this->initByID($sliderID);
	...
	private function copyMoveSlide($slideID,$targetSliderID,$operation){
		...
		if($operation == "move"){
			...
			$GlobalsRevSlider = new GlobalsRevSlider(); 
			$newOrder = $maxOrder+1;
			$arrUpdate = array("slider_id"=>$targetSliderID,"ordering"=>$newOrder);	
	...
	private function shiftOrder($fromOrder){
		...
		$sql = "update ".$GlobalsRevSlider::$table_slides." set ordering=(ordering+1) where $where";
		$this->db->runSql($sql);
	...
	public function exportSlider($useDummy = false){
		...
		//echo $GlobalsRevSlider::$urlExportZip;die;
		if($export_zip){
			$zip = new ZipArchive;
			...
			//add images to zip
			if(!empty($usedImages)){
				$upload_dir = UniteFunctionsRev::getPathUploads();
		...
		try{
			...
			if($importZip === true){ //we have a zip
				...
				if(!empty($animations)){
					...
					dmp("animations imported!");
				}else{
					dmp("no custom animations found, if slider uses custom animations, the provided export may be broken...");
				}
				...
				if(is_array($dynamicCss) && $dynamicCss !== false && count($dynamicCss) > 0){
					...
					dmp("dynamic styles imported!");
				}else{
					dmp("no dynamic styles found, if slider uses dynamic styles, the provided export may be broken...");
				}
			}
			...
			if(isset($sliderParams["background_image"]))
				$sliderParams["background_image"] = $UniteFunctionsRev::getImageUrlFromPath($sliderParams["background_image"]);
			...
			foreach($arrSlides as $slide){
				...
				if(isset($params["image"])){
					...
					if(strpos($params["image"], 'http') !== false){
							...
							if($importZip === true){ //we have a zip, check if exists
								...
								if(!$image){
									...
									if(!isset($alreadyImported['zip://'.$filepath."#".'images/'.$params["image"]])){
										$importImage = UniteFunctionsRev::import_media('zip://'.$filepath."#".'images/'.$params["image"], $sliderParams["alias"].'/');
						...
						$params["image"] = UniteFunctionsRev::getImageUrlFromPath($params["image"]);
					}
				}
				...
				foreach($layers as $key=>$layer){					
					if(isset($layer["image_url"])){
								...
								if($importZip === true){ //we have a zip, check if exists
									...
									if(!$image_url){
										...
										if(!isset($alreadyImported['zip://'.$filepath."#".'images/'.$layer["image_url"]])){
											$importImage = UniteFunctionsRev::import_media('zip://'.$filepath."#".'images/'.$layer["image_url"], $sliderParams["alias"].'/');
											if($importImage !== false){
						...
						$layer["image_url"] = UniteFunctionsRev::getImageUrlFromPath($layer["image_url"]);
						$layers[$key] = $layer;
					}
				}
				...
				$arrCreate["ordering"] = $slide["ordering"];
	...
	private function getSlidesFromPosts($publishedOnly = false){
		...
		$sourceType = "gallery";//$this->getParam("source_type","gallery");
		switch($sourceType){
			case "posts":
	...
	public function getSlidesFromGallery($publishedOnly = false){
		...
		$arrSlides = array();
		$arrSlideRecords = $this->db->fetch($GlobalsRevSlider::$table_slides,"slider_id=".$this->id,"ordering");
	...
	public function getMaxOrder(){
		...
		$GlobalsRevSlider = new GlobalsRevSlider(); 
		$maxOrder = 0;
		$arrSlideRecords = $this->db->fetch($GlobalsRevSlider::$table_slides,"slider_id=".$this->id,"ordering desc","","limit 1");
