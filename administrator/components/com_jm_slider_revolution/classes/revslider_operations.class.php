...
class RevOperations extends UniteElementsBaseRev{
	...
	public static function getWildcardsSettings(){
		...
		$arrSlides = $slider->getArrSlidersWithSlidesShort($slider::SLIDER_TYPE_TEMPLATE);
	...
	public static function getCaptionsContentArray($handle = false){
		$db = new UniteDBRev();
		$GlobalsRevSlider = new GlobalsRevSlider();
		$UniteCssParserRev = new UniteCssParserRev();
	...
	public static function updateDynamicCaptions($full = false){
		...
		if($full){
			$captions = array();
			$captions = $operations::getCaptionsContentArray();
	...
	public function previewOutput($sliderID,$output = null){
		...
		//put the output html
		$urlPlugin = JURI::base()."components/com_jm_slider_revolution/rs-plugin/";
		$u = JFactory::getURI();
		$setBase = ($u->isSSL()) ? "https://" : "http://";
	...
	public function modifyCustomSliderParams($data){
		...
		foreach($arrNames as $name){
			if(array_key_exists($name, $arrMain)){
				$arrMain[$name] = $settigns->modifyValueByDatatype($arrMain[$name], $settigns::DATATYPE_NUMBER);
			}
		}

		$arrMain["fullscreen_offset_container"] = $settigns->modifyValueByDatatype($arrMain["fullscreen_offset_container"], $settigns::DATATYPE_STRING);
		$data["main"] = $arrMain;
