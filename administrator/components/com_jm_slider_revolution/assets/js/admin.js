...
var UniteAdminRev = new function(){
	...
	********************************/
	...
	t.showInfo = function(obj) {
		...
		if (jQuery('#eg-toolbox-wrapper').length==0) jQuery('.settings_wrap').append('<div id="eg-toolbox-wrapper"></div>');
	...
	t.ajaxRequest = function(action,data,successFunction){
		...
		showAjaxLoader();
		hideAjaxButton();
		//alert(JSON.stringify(objData));return false;
		jQuery.ajax({
			type:"POST",
			url:"index.php?option=com_jm_slider_revolution&view=slider&format=slider",
			data:objData,
			success:function(response){
				//alert(response);return;
				hideAjaxLoader();
				response = jQuery.parseJSON(response);
				//if(typeof(response)=='json') response = jQuery.parseJSON(response);
				if(!response){
				...
				if(response.is_redirect==1||response.is_redirect=='1'){
					response.redirect_url = response.redirect_url.replace(new RegExp('&amp;', 'g'), '&');
