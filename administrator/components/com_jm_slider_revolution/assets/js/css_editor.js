...
var UniteCssEditorRev = new function(){	
	...
	var urlCssCaptions = jQuery('#urlCssCaptions').val();
	...
	var setCssPreview = function(){
		...
		if(typeof curActiveStyles === 'object' && !jQuery.isEmptyObject(curActiveStyles)){
			for(var attr in curActiveStyles){
				if(attr == 'position' || attr == '') continue; //ignore position absolute
	...
	t.setCssPreviewLive = function(){
		if(cssExpertEditor){
			...
			//add novice styles
			jQuery('#css_preview').removeAttr('style'); 
			...
			if(jQuery('input[name="css_background-color"]').val() != 'transparent'){
				...
				jQuery('input[name="css_background-transparency"]').val(transparency);
				curActiveStyles['background-color'] = 'rgb('+rgb[0]+', '+rgb[1]+', '+rgb[2]+')';
				curActiveStyles['background-transparency'] = transparency;
	...
	var setActiveStylesFromExample = function(){
		...
		jQuery.each(rawStyles, function( index, value ) {
			var temp = new String(value);
			temp = temp.split(':');
			cur_attr = jQuery.trim(temp[0]);
			cur_style = temp[1];
			if(jQuery.trim(temp[0]) == '' || jQuery.trim(temp[1]) == '') {
				if(temp[0].toLowerCase().indexOf("border") >= 0){
		...
		//alert(JSON.stringify(curActiveStyles));
		//handle borders
	...
	var setPreviewFromExpert = function(){
		...
		alert(curActiveStyles['background-color']);
	...
	t.setCssStylesExpert = function(ignoreExample){
		...
		for(var attr in curActiveStyles){
			if(jQuery.trim(curActiveStyles[attr]) == '' || attr == '') continue;
			if(attr == 'background-color' && curActiveStyles[attr] !== 'transparent'){
				...
				if(curActiveStyles[attr]!= null ||curActiveStyles[attr]!= '')
				cssData += '	'+attr+': '+curActiveStyles[attr]+";\n";
			}
		}
	...
	t.setCssStylesNovice = function(ignoreExample){
		...
		if(typeof ignoreExample === 'undefined') setActiveStylesFromExample();
		//set styles from example
	...
	var updateStylesInDb = function(handle, id){
		...
		jQuery.each(curFullClass, function(i, val){
			if(i=='params'){
				...
				jQuery.each(newObj,function(e,val){
					newObj[e] = addslashes(val);
				})
				console.log(newObj);
				temp[i] = newObj;
			}
		...
		if(id === false){ //create new
			...
			UniteAdminRev.ajaxRequest("insert_captions_css",temp,function(response){
				...
				//update html select (got as "data" from response)
				updateCaptionsInput(response.data);
				jQuery('#rs-plugin-captions-css').html(response.csscontent);
			});
