...
var UniteLayersRev = new function(){
	...
	t.setInitLayerAnim = function(jsonLayersAnims){
		if(typeof(jsonLayersAnims)=='json')
	...
	var updateAnimInDb = function(handle, animObj, id){
		...
		if(id === false){ //create new
			...
			UniteAdminRev.ajaxRequest("insert_custom_anim",animObj,function(response){
				...
				//update html select (got from response)
				t.updateInitLayerAnim(response.data.customfull);
				updateLayerAnimsInput(response.data.customin, 'customin');
				updateLayerAnimsInput(response.data.customout, 'customout');
...
/*					stop: function() {
	 ...
	 * add image layer
	 */
	t.addLayerImage = function(urlImage, special_type){
